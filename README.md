...
XHS_BATCH_REQUEST_INTERVAL_SECONDS=4
XHS_BATCH_ACCOUNT_DELAY_SECONDS=2
XHS_BATCH_ACCOUNT_JITTER_SECONDS=1.5
XHS_BATCH_CHUNK_SIZE=5
XHS_BATCH_CHUNK_COOLDOWN_SECONDS=25
XHS_BATCH_RETRY_FAILED_ONCE=true
XHS_BATCH_RETRY_DELAY_SECONDS=35
XHS_BATCH_RISK_RETRY_DELAY_SECONDS=90
XHS_BATCH_PROJECT_COOLDOWN_SECONDS=75
XHS_BATCH_PRESSURE_CONSECUTIVE_THRESHOLD=2
XHS_BATCH_PRESSURE_COOLDOWN_SECONDS=90
XHS_BATCH_SLOWDOWN_MULTIPLIER=2
XHS_BATCH_SIGNED_PROFILE_PAGE_CAP=12
XHS_BATCH_WORK_METRIC_LIMIT=12
```
...
- `XHS_BATCH_CHUNK_SIZE / XHS_BATCH_CHUNK_COOLDOWN_SECONDS` 会在连续抓完一小段账号后主动停一下，避免单次会话在短时间内累计太多连续请求
...
- `XHS_BATCH_PRESSURE_CONSECUTIVE_THRESHOLD / XHS_BATCH_PRESSURE_COOLDOWN_SECONDS / XHS_BATCH_SLOWDOWN_MULTIPLIER` 会在连续命中“空结果 / 登录跳转 / 验证 / 风控”时自动切换到慢速模式，而不是继续用原节奏硬冲
- `XHS_BATCH_SIGNED_PROFILE_PAGE_CAP` 会限制批量任务里单账号最多额外翻多少页签名接口，避免为了精确作品总数把请求量拉得过高
- `XHS_BATCH_WORK_METRIC_LIMIT` 会限制批量任务里需要补抓详情/评论的作品条数，默认只强化前 `12` 条，剩余作品沿用主页卡片数据，避免单账号拉出几十次详情请求
