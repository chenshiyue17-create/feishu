...
from xhs_feishu_monitor.profile_batch_to_feishu import LoginFailureDuringSync
...
class LocalStatsAppTest(unittest.TestCase):
    ...
    def test_save_uploaded_server_cache_restores_deleted_dashboard_from_cloud_backup(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            env_path = Path(temp_dir) / ".env"
            urls_path = Path(temp_dir) / "urls.txt"
            cache_dir = Path(temp_dir) / "cache"
            ...
            save_uploaded_server_cache(env_file=str(env_path), urls_file=str(urls_path), payload=first_payload)
            (cache_dir / "dashboard_all.json").unlink()
            ...
            self.assertIn("2026-04-20", payload["history_rankings"]["默认项目"])
            self.assertIn("2026-04-24", payload["history_rankings"]["默认项目"])
            self.assertTrue((cache_dir / "server_cloud_backups").exists())
    ...
    def test_sync_loop_manual_partial_success_auto_uploads_only_successful_accounts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            path = write_monitored_entries(
                f"{temp_dir}/urls.txt",
                [
            ...
            with (
                ...
                patch.object(store, "_ensure_login_ready_for_sync", return_value=None),
                patch("xhs_feishu_monitor.local_stats_app.server.load_reports_for_sync", return_value=reports),
                patch("xhs_feishu_monitor.local_stats_app.server.write_project_cache_bundle", return_value=None),
                patch(
                    "xhs_feishu_monitor.local_stats_app.server.load_cached_dashboard_payload",
                    return_value={"generated_at": "2026-03-23T18:05:00+08:00", "latest_date": "2026-03-23"},
                ),
    ...
    def test_sync_loop_opens_login_window_and_preserves_partial_success_on_login_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch(
                    ...
                    side_effect=LoginFailureDuringSync(
                        "检测到登录态异常：https://www.xiaohongshu.com/user/profile/u2 命中登录页，当前登录态不可用",
        ...
        self.assertEqual(store._status["summary"]["successful_accounts"], 1)
        self.assertEqual(store._status["summary"]["failed_accounts"], 1)
        self.assertIn("已成功保留 1 个账号", store._status["message"])
    ...
    def test_sync_loop_auto_partial_success_uploads_only_successful_accounts(self) -> None:
        ...
        push_mock.assert_called_once_with(auto=True, account_ids=["u1"])
        self.assertEqual(store._status["summary"]["failed_accounts"], 1)
